        ...
        if 'value' in request.form:
            ...
            if(received_value=='1'):
                ...
                try: 
                    ...
                    if 'items' in results:
                        snippets = [item['snippet'] for item in results['items'][:5]]  # Take only the first 5 snippets
                    ...
                    response = chat.send_message(inital_conversation_prompt+prompt+". The google search result for the above prompt is as given below please answer accordingly using this as reference:"+search_res+"/n/n The current time is "+time_text,stream=True)
                except:
                    chat.rewind()
